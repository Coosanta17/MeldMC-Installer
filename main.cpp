...
void MeldInstaller::fetchVersions()
{
    try {
        ...
        try {
            // Fetch releases
            std::string releasesXml = downloadUrl("https://repo.coosanta.net/releases/net/coosanta/meldmc/maven-metadata.xml");
            releases = parseVersionsFromXml(releasesXml);
            
            // Fetch snapshots  
            std::string snapshotsXml = downloadUrl("https://repo.coosanta.net/snapshots/net/coosanta/meldmc/maven-metadata.xml");
    ...
    try {
        ...
        try {
            ...
            nlohmann::json mockClient = {
                ...
                {"downloads", {
                    {"client", {
                        {"url", "https://repo.coosanta.net/releases/net/coosanta/meldmc/" + version.toStdString() + "/meldmc-" + version.toStdString() + "-client.jar"},
                        {"sha1", ""},
                        {"size", 0}
                    }}
                }}
